...
class SACN(torch.nn.Module):
    def __init__(self, num_entities, num_relations):
        ...
        self.emb_e = torch.nn.Embedding(num_entities, Config.init_emb_size, padding_idx=0)
        # emb_e_size = 'The dimension size of emb_e is :{0}'
        # print(emb_e_size.format(self.emb_e.size()))

        self.gc1 = GraphConvolution(Config.init_emb_size, Config.gc1_emb_size, num_relations)
        # gc1_size = 'The dimension size of gc1 is :{0}'
        # print(gc1_size.format(self.gc1.size()))

        self.gc2 = GraphConvolution(Config.gc1_emb_size, Config.embedding_dim, num_relations)
        # gc2_size = 'The dimension size of gc2 is :{0}'
        # print(gc2_size.format(self.gc2.size()))
    ...
    def forward(self, e1, rel, X, A):
        ...
        # info_gc1 = 'After gc1 convolution :{0}'
        # print(info_gc1.format(x.size()))

        x = self.bn3(x)
        # info_bn3 = 'After bn3 operation :{0}'
        # print(info_bn3.format(x.size()))

        x = x.tanh()
        x = F.dropout(x, Config.dropout_rate, training=self.training)

        x = self.bn4(self.gc2(x, A))
        # info_gc2_bn4 = 'After gc2 & bn4 operation :{0}'
        # print(info_gc2_bn4.format(x.size()))

        e1_embedded_all = x.tanh()
